...
const lookup = async ({
  ...
  fields = [...fields.filter(t => t !== "cmd"), "cmd"];
  ...
  throw new Error(
    'ps - must provide a search query; one of "pid", "ppid", "all", "user", "group", "command"'
  );
};
  ...
  return lines.map(line => {
    ...
    line
      ...
      .forEach((item, index) => {
        if (item) {
          if (header[index]) {
            lHeader = header[index];
          }
          proc[lHeader] = proc[lHeader] ? proc[lHeader] + " " + item : item;
        }
      });
